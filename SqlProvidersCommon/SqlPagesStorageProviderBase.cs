...
namespace ScrewTurn.Wiki.Plugins.SqlCommon {
    ...
    public abstract class SqlPagesStorageProviderBase : SqlStorageProviderBase, IPagesStorageProviderV30
    {
        ...
        {
            return new SqlWordFetcher(GetCommandBuilder().GetConnection(connString), TryFindWord, ExpandQueryWords);
        }
        ...
        private void ExpandQueryWords(ref string[] queryWords, DbConnection connection)
        {
            ...
            foreach (var word in queryWords)
            {
                ...
                string query = queryBuilder.SelectFrom("IndexWord", new string[] { "Text" });
                query = queryBuilder.Where(query, "Text", WhereOperator.Like, "Text");

                var parameters = new List<Parameter>(1) { new Parameter(ParameterType.String, "Text", "%" + word + "%") };
                ...
                while (x.Read())
                {
                    newWords.Add(x.GetString(0));
                }

                CloseReader(x);
            }

            newWords.AddRange(queryWords);

            queryWords = newWords.Distinct().Where(q => q.Length >= 3).ToArray();
        }
        ...
        private bool TryFindWord(string text, out Word word, DbConnection connection)
        {
            ...
            ////var qq = new List<int>();
            ////var x = command.ExecuteReader();
            ////while (x.Read()) qq.Add(x.GetInt32(0));
            ...
            ////wordId = ExecuteScalar<int>(command, -1, false);
