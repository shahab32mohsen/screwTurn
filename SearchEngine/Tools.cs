...
namespace ScrewTurn.Wiki.SearchEngine {
    ...
    public static class Tools
    {
        ...
        public static SearchResultCollection SearchInternal(string query, string[] documentTypeTags, bool filterDocumentType, SearchOptions options, IWordFetcher fetcher)
        {
            ...
            queryWords = queryWords.Where(q => q.Length >= 3).ToArray();

            var queryWordsCopy = queryWords;

            if (SearchParameters.Substrings)
                ExpandQueryWords(ref queryWords, fetcher);
            ...
            foreach (string q in queryWords)
            {
                ...
                if (fetcher.TryGetWord(q, out word))
                {
                    foreach (IDocument doc in word.Occurrences.Keys)
                    {
                        ...
                        if (filterDocumentType && !IsDocumentTypeTagIncluded(doc.TypeTag, documentTypeTags))
                        {
                            continue;
                        }
            ...
            switch (options)
            {
                case SearchOptions.AllWords:
                    totalRelevance -= PurgeResultsForAllWords(ref results, queryWordsCopy, fetcher);
                    break;
                case SearchOptions.AtLeastOneWord:
                    // Nothing to do
                    break;
                case SearchOptions.ExactPhrase:
                    totalRelevance -= PurgeResultsForExactPhrase(results, queryWordsCopy);
                    break;
                default:
            ...
            foreach (SearchResult t in results)
            {
                t.Relevance.Finalize(totalRelevance);
            }

            return results;
        }

        private static void ExpandQueryWords(ref string[] queryWords, IWordFetcher fetcher)
        {
            fetcher.ExpandQueryWords(ref queryWords);
        }
        ...
        public static float PurgeResultsForAllWords(ref SearchResultCollection results, string[] queryWords, IWordFetcher fetcher)
        {
            // Remove results that do not contain all the searched words
            var wordsPlus = new List<List<string>>();

            foreach (var word in queryWords)
            {
                var newList = new[] { word };

                fetcher.ExpandQueryWords(ref newList);

                wordsPlus.Add(newList.ToList());
            }

            float relevanceToRemove = 0;
            var toRemove = (from r in results let r1 = r let containsAll = wordsPlus.Select(wordPlus => wordPlus.Any(w => r1.Matches.Contains(w))).All(containsLocal => containsLocal) where !containsAll select r).ToList();
        ...
        public static bool IsDocumentTypeTagIncluded(string currentTag, string[] includedTags)
        {
            ...
            return includedTags.Any(s => s.ToLowerInvariant() == currentTag);
        }
    ...
    public interface IWordFetcher : IDisposable
    {
        ...
        /// <summary>
        /// Expanding Query Words collection, adding all words, that contains words in collection
        /// </summary>
        void ExpandQueryWords(ref string[] queryWords);
    }
